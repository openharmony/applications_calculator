...
## 简介
`Calculator` 是 OpenHarmony 系统中的基础计算器应用，提供标准计算器和科学计算器功能，并支持历史记录管理。该应用采用 ArkTS 语言开发，基于 OpenHarmony Stage 模型，支持 Phone、Tablet 和 PC 多种设备形态，具备响应式布局、多设备适配和无障碍支持等特性。

Calculator 包含以下常用功能：

* **标准计算器**：支持基本的四则运算、百分比、平方根等常用计算功能。
* **科学计算器**：支持三角函数、对数、指数、阶乘等高级数学运算。
* **历史记录**：自动保存计算历史，支持查看、复制和删除历史记录。
...
* **开发环境**
  ...
  * **SDK 版本**: OpenHarmony SDK API Version 12
  * **语言版本**: ArkTS

* **运行平台**
  * 仅支持标准系统
  * 支持 Phone、Tablet、PC 设备形态
  * 支持折叠屏设备
